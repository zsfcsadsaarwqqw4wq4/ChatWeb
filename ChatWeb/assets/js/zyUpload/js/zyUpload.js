...
(function ($, undefined) {
    $.fn.zyUpload = function (options, param) {
        ...
        return this.each(function () {
            ...
            var acceptType = function (tp) {
                var img = ["jpeg", "jpg", "jpe", "tif", "tiff", "gif", "jp2", "png"];
                var txt = ["css", "csv", "htm", "html", "javascript", "txt", "xml"];
                if (img.indexOf(tp) > 0) {
                    if (img.indexOf(tp) < 3) {
                        tp = "jpeg";
                    } else if (img.indexOf(tp) == 3) {
                        tp = "tiff";
                    }
                    return "image/" + tp;
                }
            ...
            this.createHtml = function () {
                ...
                if (para.dragDrop) {
                    ...
                    html += '						<div class="filePicker">Select Img</div>';
                    html += '						<input id="fileImage" type="file" size="30" name="fileselect[]" ' + multiple + ' accept="';
                    for (var i = 0; i < para.fileType.length; i++) {
                        html += acceptType(para.fileType[i]);
                        if (i < para.fileType.length - 1) {
                            html += ",";
                        }
                    }
                    html += '">';
                    html += '					</div>';
                    html += '				</div>';
                    html += '				<span id="fileDragArea" class="upload_drag_area">Or drag the images here</span>';
                    html += '			</div>';
                    html += '			<div class="status_bar">';
                    html += '				<div id="status_info" class="info am-hide-sm-only">0 images - 0B</div>';
                    html += '				<div class="btns">';
                    html += '					<div class="webuploader_pick">Continue selection</div>';
                    ...
                    html += '			<button type="button" id="fileSubmit" class="upload_submit_btn">Submit</button>';
                    html += '		</div>';
                    ...
                    html += '			<div class="status_bar">';
                    html += '				<div id="status_info" class="info am-hide-sm-only" style="display:block;">0 images - 0B</div>';
                    html += '				<div class="btns">';
                    html += '					<input id="fileImage" type="file" size="30" name="fileselect[]" ' + multiple + ' accept="';
                    for (var i = 0; i < para.fileType.length; i++) {
                        html += acceptType(para.fileType[i]);
                    ...
                    html += '">';
                    html += '					<div class="webuploader_pick">Select Img</div>';
                    html += '					<div class="upload_btn">Upload</div>';
                    html += '				</div>';
                    html += '			</div>';
                    html += '			<div id="preview" class="upload_preview">';
                    html += '				<div class="add_upload">';
                    html += '					<a style="height:' + para.itemHeight + ';width:' + para.itemWidth + ';" title="Click to add Images" id="rapidAddImg" class="add_imgBox" href="javascript:void(0)">';
                    html += '						<div class="uploadImg" style="width:' + imgWidth + 'px">';
                    html += '							<img class="upload_image" src="/assets/i/zyUpload/add_img.png" style="width:expression(this.width > ' + imgWidth + ' ? ' + imgWidth + 'px : this.width)" />';
                    html += '						</div>';
                    html += '					</a>';
                    html += '				</div>';
                    html += '			</div>';
            ...
            this.funSetStatusInfo = function (files) {
                ...
                if (size > 1024 * 1024) {
                    size = (Math.round(size * 100 / (1024 * 1024)) / 100).toString() + 'M';
                } else {
                    size = (Math.round(size * 100 / 1024) / 100).toString() + 'K';
                }

                // 设置内容
                $("#status_info").html(num + " images - " + size + "B");
            };
            ...
            this.funDisposePreviewHtml = function (file, e) {
                ...
                for (var i = 0; i < para.fileType.length; i++) {
                    ...
                    if (fileNames[fileNames.length - 1] == para.fileType[i]) {
                        isCorrectType = true;
                        break;
                    }
                }
                if (!isCorrectType) {
                    alert("The File【" + file.name + "】format is incorrect");
                    ZYFILE.funDeleteFile(parseInt(file.index), true);
                    return html;
                }
                var imgWidth = parseInt(para.itemWidth.replace("px", "")) - 15;
                ...
                // 处理不同类型文件代表的图标
                var fileImgSrc = "/assets/i/zyUpload/fileType/";
                ...
                if (file.type.indexOf("image") == 0) {
                    ...
                    html += '		<div class="uploadImg" style="width:' + imgWidth + 'px">';
                    html += '			<img id="uploadImage_' + file.index + '" class="upload_image" src="' + e.target.result + '" style="width:' + para.itemWidth + ' !important; height:' + para.itemHeight + ' !important;" />';
                    html += '		</div>';
                    html += '	</a>';
                    ...
                    html += '	<a style="height:' + para.itemHeight + ';width:' + para.itemWidth + ';" href="javascript:void(0);" class="imgBox">';
                    html += '		<div class="uploadImg" style="width:' + imgWidth + 'px">';
                    html += '			<img id="uploadImage_' + file.index + '" class="upload_image" src="' + fileImgSrc + '" style="width:' + para.itemWidth + ' !important; height:' + para.itemHeight + ' !important;" />';
                    html += '		</div>';
                    html += '	</a>';
                    html += '	<p id="uploadProgress_' + file.index + '" class="file_progress"></p>';
                    html += '	<p id="uploadFailure_' + file.index + '" class="file_failure">Upload failed, please try again</p>';
            ...
            this.addEvent = function () {
                ...
                $(".upload_btn").bind("click", function (e) {
                    ...
                    if (ZYFILE.funReturnNeedFiles().length > 0) {
                        ...
                        alert("Please select at least one file then click upload");
                    }
                });
